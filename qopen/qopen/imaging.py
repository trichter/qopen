...
def plot_lstsq(rec,  event_station_pairs, coda_normalization=None, 
               ax=None, base=np.e, **kwargs):
...
def plot_optimization(record, record_g0, event_station_pairs, 
                      coda_normalization=None, num=7, **kwargs):
    ...
    if num > 1:
        for i, rec in enumerate(record):
            ...
            plot_lstsq(rec, event_station_pairs, coda_normalization, ax=ax2, **kwargs)
...
def plot_fits(energies, g0, b, W, R, v0, info, G_func,
    ...
    for i, energy in enumerate(energies):
        ...
        plot = ax.semilogy
        def get_Emod(G, t, coda_normalization):
            if coda_normalization is None:
                return R[station] * W[evid] * G * np.exp(-b * t)
            else:
                return W[evid] * G * np.exp(-b * t)
#            return R[station] * W[evid] * G * np.exp(-b * t)
